...
- [x] Competing ordinary-path couriers now have bounded single-owner supply leases. Partial physical stock, lease expiry and reacquisition, stale completion, idempotent retry, monotonic revisions, and exact source/cargo/requester/demand conservation are covered without private controller calls or post-assignment teleportation.
...
- [x] Fresh automated verification on 2026-08-01 passed `runHarnesses` (176 actionable tasks), two consecutive NeoForge runs with all 79 required GameTests passing, `buildAll` for both Fabric and NeoForge (186 actionable tasks), and `git diff --check`.
...
- [ ] Complete the remaining courier edge cases: exact-transfer rollback under runtime failure, manual/automatic/hybrid route transitions, settlement removal, unload/reload during a live lease, and the equivalent checks with real connected clients.
