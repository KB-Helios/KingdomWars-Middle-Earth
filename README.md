...
- [x] Fresh automated verification on 2026-08-01 passed `runHarnesses` (176 actionable tasks), two consecutive NeoForge runs with all 80 required GameTests passing, `buildAll` for both Fabric and NeoForge (186 actionable tasks), and `git diff --check`.
...
- [ ] Complete the remaining courier edge cases: manual/automatic/hybrid route transitions, settlement removal during an active route or lease, and the equivalent checks with real connected clients.
